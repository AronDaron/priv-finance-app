...
### Strategia testowania (dev vs produkcja)
- **`npm run dev` (przeglądarka):** Vite dev server (`--host`) dostępny w przeglądarce pod IP serwera LXC. Dane persystowane przez `localStorage`. Warstwa `src/lib/api.ts` automatycznie wykrywa brak `window.electronAPI` i używa localStorage jako backend — pełne UI działa w przeglądarce bez Electrona.
- **`.exe` (produkcja):** Te same komponenty React, ta sama warstwa `api.ts`, ale `window.electronAPI` istnieje → dane zapisywane w lokalnej bazie SQLite przez `better-sqlite3`.
- **Zasada:** Komponenty React nigdy nie wywołują `window.electronAPI` bezpośrednio — zawsze przez `src/lib/api.ts`.
...
- [ ] **3. Integracja Danych:** Zaimplementuj pobieranie danych z `yahoo-finance2` (ceny, OHLC, fundamentalne, wyszukiwarka) przez IPC. W trybie dev (przeglądarka) — mock dane historyczne. Wyświetl wykres świecowy Lightweight Charts dla dowolnego tickera.
